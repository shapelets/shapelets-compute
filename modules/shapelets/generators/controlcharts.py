...
def _run_choice(py_rnd: random.Random, sh_rnd: sc.random.ShapeletsRandomEngine, n: int, choices: List,
                prob: List) -> sc.ShapeletsArray:
...
def _process_single_program_instruction(x: DeltaGenerator, n: int, py_rnd: random.Random,
                                        sh_rnd: sc.random.ShapeletsRandomEngine):
    return (lambda z, e: lambda: _run_single(sh_rnd, z, e))(n, x)


def _process_multiple_program_instruction(x: List, n: int, py_rnd: random.Random,
                                          sh_rnd: sc.random.ShapeletsRandomEngine):
    l = len(x)
    prob = [1.0 / l] * l
...
def _process_program_instruction(x: Union[List, DeltaGenerator], n: int, py_rnd: random.Random,
                                 sh_rnd: sc.random.ShapeletsRandomEngine):
