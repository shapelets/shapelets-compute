...
void device_bindings(py::module &m) {
    ...
    py::class_<DeviceInfo>(m, "DeviceInfo", R"__d__(
        ...
        A computational backend may expose more than one device.  Use ::func::`~khiva.get_devices` to
    ...
    py::class_<DeviceMem>(m, "DeviceMemory", R"__d__(
        ...
        Use ::func::`~khiva.get_device_memory` to report and populate memory usage. The function
        ::func::`~khiva.device_gc` will force a synchronization and removal of temporal arrays.
        )__d__")
    ...
    m.def("set_device",
        ...
        To select the new device, one could either use a ::class::`khiva.DeviceInfo` or the `id`
    ...
    m.def("get_device_memory",
        ...
        current device; however, one could either use a ::class::`khiva.DeviceInfo` or the `id`
