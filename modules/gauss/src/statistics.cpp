...
af::array gauss::statistics::covariance(const af::array &tss, bool unbiased) {
    ...
    af::array result = gauss::features::crossCovariance(tss, tss) * n / (n - unbiased);
...
af::array gauss::statistics::kurtosis(const af::array &tss) {
    ...
    af::array sampleStdev = af::tile(gauss::statistics::sampleStdev(tss), static_cast<unsigned int>(tss.dims(0)));
...
af::array gauss::statistics::moment(const af::array &tss, int k) {
    auto n = static_cast<double>(tss.dims(0));

    return af::sum(af::pow(tss, k), 0) / n;
}

af::array gauss::statistics::ljungBox(const af::array &tss, long lags) {
    auto n = tss.dims(0);
    const double e = 2;
    af::array ac = gauss::features::autoCorrelation(tss, lags + 1);
...
af::array gauss::statistics::sampleStdev(const af::array &tss) {
    auto n = static_cast<double>(tss.dims(0));
...
af::array gauss::statistics::skewness(const af::array &tss) {
    ...
    af::array s3 = af::pow(gauss::statistics::sampleStdev(tss), 3);
