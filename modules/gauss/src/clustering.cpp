...
float computeError(const af::array &means, const af::array &newMeans) {
    auto error = gauss::utils::makeScopedHostPtr(
...
af::array shapeExtraction(const af::array &tss, const af::array &centroid) {
    ...
    shiftedTSS = gauss::normalization::znorm(shiftedTSS);
    ...
    first = af::select(af::tile(condition, first.dims(0), 1), gauss::normalization::znorm(first * (-1)),
                       gauss::normalization::znorm(first));
...
void gauss::clustering::kMeans(const af::array &tss, int k, af::array &centroids, af::array &labels, float tolerance,
                               int maxIterations) {
...
void gauss::clustering::kShape(const af::array &tss, int k, af::array &centroids, af::array &labels, float tolerance,
                               int maxIterations) {
    ...
    af::array normTSS = gauss::normalization::znorm(tss);
