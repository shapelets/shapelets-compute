...
#include <gauss/internal/vectorUtil.h>
...
namespace {
using namespace gauss::matrix::internal;
...
void massWithMask(af::array q, const af::array &t, const af::array &a, const af::array &mean_t,
    ...
    q = gauss::normalization::znorm(q, EPSILON);
...
void scamp(af::array ta, af::array tb, long m, af::array &profile, af::array &index) {
    ...
    for (dim_t tbIdx = 0; tbIdx < tb.dims(1); ++tbIdx) {
        for (dim_t taIdx = 0; taIdx < ta.dims(1); ++taIdx) {
            auto vectA = gauss::vectorutil::get<double>(ta(af::span, taIdx));
            auto vectB = gauss::vectorutil::get<double>(tb(af::span, tbIdx));
            auto res = ::scamp(std::move(vectB), std::move(vectA), m);
            profile(af::span, taIdx, tbIdx) = gauss::vectorutil::createArray<double>(res.first);
            index(af::span, taIdx, tbIdx) = gauss::vectorutil::createArray<unsigned int>(res.second);
        }
    }
}
...
void scampLR(af::array tss, long m, af::array &profileLeft, af::array &indexLeft, af::array &profileRight,
    ...
    for (dim_t tssIdx = 0; tssIdx < tss.dims(1); ++tssIdx) {
        ...
        profileLeft(af::span, tssIdx) = gauss::vectorutil::createArray<double>(res.first.first);
        indexLeft(af::span, tssIdx) = gauss::vectorutil::createArray<unsigned int>(res.first.second);
        profileRight(af::span, tssIdx) = gauss::vectorutil::createArray<double>(res.second.first);
        indexRight(af::span, tssIdx) = gauss::vectorutil::createArray<unsigned int>(res.second.second);
    }
...
void stomp_batched(const af::array &ta, af::array tb, long m, long batch_size, af::array &profile, af::array &index) {
    ...
    gauss::matrix::internal::meanStdev(ta, aux, m, mean, stdev);
    ...
    for (long i = 0; i < (nb - m + 1); i += chunkSize) {
        ...
        gfor(af::seq idx, iterationSize) {
            ...
            gauss::matrix::internal::mass(input(af::span, idx, af::span, af::span), ta, aux, mean, stdev, distances);
...
void stomp_batched_two_levels(af::array ta, af::array tb, long m, long batch_size_b, long batch_size_a,
    ...
    for (long i = 0; i < (nb - m + 1); i += chunkSizeB) {
        ...
        for (long start = 0; end < na - 1; start += (iterationSizeA - m + 1)) {
            ...
            gauss::matrix::internal::meanStdev(taChunk, aux, m, mean, stdev);
            // For all the subsequences in input for the given batch
            gfor(af::seq idx, iterationSizeB) {
                ...
                // Compute the distance and index profiles using Mueens algorithm for similarity search
                gauss::matrix::internal::mass(input(af::span, idx, af::span, af::span), taChunk, aux, mean, stdev,
                                              distancesTmp);
...
void stomp_batched_two_levels(af::array t, long m, long batch_size_b, long batch_size_a, af::array &profile,
    ...
    for (long i = 0; i < (n - m + 1); i += chunkSizeB) {
        ...
        for (long start = 0; end < n - 1; start += (iterationSizeA - m + 1)) {
            ...
            af::array stdev;
            // Computing the mean and standard deviation of all the reference time series contained in taChunk
            gauss::matrix::internal::meanStdev(tChunk, aux, m, mean, stdev);
            // For all the subsequences in input for the given batch
            gfor(af::seq idx, iterationSizeB) {
                af::array distancesTmp;
                af::array minDistanceTmp;
                ...
                auto mask = gauss::matrix::internal::generateMask(m, iterationSizeB, i, iterationSizeA - m + 1, start,
                                                                  nTimeSeries);
                // Compute the distance and index profiles using Mueens algorithm for similarity search
                gauss::matrix::internal::massWithMask(input(af::span, idx, af::span, af::span), tChunk, aux, mean,
...
void stomp_parallel(af::array t, long m, af::array &profile, af::array &index) {
    ...
    // Computing the mean and standard deviation of all the reference time series contained in ta
    gauss::matrix::internal::meanStdev(t, aux, m, mean, stdev);
    ...
    // Calculating the mask required to filter the trivial matches
    auto mask = gauss::matrix::internal::generateMask(m, n - m + 1, 0, n - m + 1, 0, nTimeSeries);

    // For all the subsequences of tb
    gfor(af::seq idx, n - m + 1) {
        af::array distances;
        // Compute the distance and index profiles using Mueens algorithm for similarity search
        gauss::matrix::internal::massWithMask(input(af::span, idx, af::span, af::span), t, aux, mean, stdev, mask,
                                              distances);
